...
原理は単純で，認証情報が保存されるディレクトリをホストマシンのディレクトリにマウントすればよい，というものです．
ちなみに，この方法は Claude Code にもそのまま適用可能です．
...
```jsonc
{
  ...
  "id": "agent-creds",
  "version": "1.0.0",
  "description": "Persist coding agent credentials (Claude Code and Codex) across dev container rebuilds using Docker volumes",
  "mounts": [
    {
      "source": "claude-creds",
      "target": "/mnt/.claude",
...
{
  "dev.containers.defaultFeatures": {
    "ghcr.io/fjktkm/devcontainer-features/agent-creds:latest": {},
  },
}
```
...
この記事で紹介した Dev Container Feature のソースコードは以下の GitHub のリポジトリで公開しています．
