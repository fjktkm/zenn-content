...
この記事では，主に設計理念や考え方に焦点を当てて説明しています．実際の具体的な手順については，Zenn Book「情報系のための Dev Container による LaTeX 環境構築」で別途解説していますので，興味のある方はぜひご覧ください．

https://zenn.dev/fjktkm/books/8bec1228335d98
...
ただ，イメージサイズが大きいという批判はありうると思います．`texlive/texlive` イメージにはスキームの異なる複数のタグが用意されています．
...
サイズを抑えたい場合は，上表の小さいスキームのタグを使い，`devcontainer.json` の `postCreateCommand` で `tlmgr` を使って必要なパッケージだけを追加する方法が適切だと思います．野良イメージに頼らずとも，公式イメージのまま柔軟にサイズを制御できます．
...
https://github.com/fjktkm/latex-devcontainer/blob/b1a65f26b89ac9f267249ca936278ee6dfc30ad4/.devcontainer/devcontainer.json#L2-L14

`"image": "texlive/texlive:latest"` で公式の TeX Live イメージを指定しています．たったこれだけです．野良 Docker イメージもお手製 Dockerfile も不要です．

`texlive/texlive` イメージには Git が入っていないので，`"features"` セクションで `common-utils` を指定して Git をインストールするようにしています．`common-utils` は Microsoft 謹製の Dev Container とターミナルの操作感を揃えるための Feature で，Git 以外にもいろいろなコマンドラインツールやシェルの設定を追加してくれます．`username` オプションには，イメージにあらかじめ用意されている非ルートユーザー `texlive` を指定しています．
...
https://github.com/fjktkm/latex-devcontainer/blob/b1a65f26b89ac9f267249ca936278ee6dfc30ad4/.devcontainer/devcontainer.json#L4-L9
...
以下の GitHub のテンプレートリポジトリでソースコードを公開しています．

https://github.com/fjktkm/latex-devcontainer

また，Dev Container の構成ファイルを追加するときに選択できるテンプレート集として，以下のリポジトリも公開しています．

https://github.com/fjktkm/devcontainer-templates

## 参考資料

提案手法の具体的な手順については，Zenn Book「Dev Container で完全に理解する LaTeX 環境構築」で別途解説していますのでぜひご覧ください．
