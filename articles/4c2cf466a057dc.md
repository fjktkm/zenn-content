...
Claude Code の CLI は公式の Feature が用意されているのでそちらを利用するのがよいでしょう．Codex CLI と Gemini CLI は公式の Feature がないため，コミュニティが提供されている `npm-packages` Feature を利用してインストールするのがよいと思います．インストールにはそこそこ時間がかかるため，どうしても CLI が使いたい場合以外は拡張機能だけで十分だと思います．
