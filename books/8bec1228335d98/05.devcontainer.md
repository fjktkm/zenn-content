...
## その他の Dev Container の操作

Dev Container で開いたプロジェクトをローカルで再度開きたい場合は，次の手順を実行してください．

1. VS Code のウィンドウの左下にある「><」のアイコンをクリックする
2. 表示されたメニューの中から「フォルダーをローカルで再度開く」を選択する

また，構成ファイルの変更はコンテナをリビルドしないと反映されないことに注意してください．Dev Container のリビルドしたい場合は，次の手順を実行してください．
